...
1. 임포트: from backend.orchestrator.screens.hypothesis_screen import HypothesisScreen, Hypothesis
...
class HypothesisScreen(ModalScreen[HypothesisSelectionResult]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="hypothesis-modal"):
            yield Label("💡 Hypothesis Selection - Choose and Edit Hypotheses", id="modal-title")
            ...
            with Horizontal(id="main-layout"):
                ...
                with Vertical(id="left-panel"):
                    ...
                    with Vertical(id="edit-container") as edit_container:
                        yield Label("📝 Edit Hypothesis:", classes="section-label")
                        yield Input(
                            id="edit-input",
                            placeholder="가설 내용을 수정하세요..."
                        )
                        yield Button("💾 Save Edit (S)", id="save-edit-btn")

                    # Priority Container (hidden by default)
                    with Horizontal(id="priority-container") as priority_container:
                        yield Label("⭐ Priority (0-10):", classes="section-label")
                        yield Input(
                            id="priority-input",
                            placeholder="0-10",
                            max_length=2
                    ...
                    with Vertical(id="add-container") as add_container:
                        yield Label("➕ Add Custom Hypothesis:", classes="section-label")
                        yield Input(
                            id="add-input",
                            placeholder="새로운 가설을 입력하세요..."
                        )
                        yield Button("➕ Add to List", id="add-btn")
    ...
    def _populate_hypothesis_list(self) -> None:
        ...
        for idx, hyp in enumerate(self.hypotheses):
            ...
            # Edited indicator
            edited_badge = "[yellow]✎[/yellow]" if hyp.edited else ""
    ...
    def _show_hypothesis_details(self, hypothesis: Hypothesis) -> None:
        ...
        # Hypothesis ID
        content.append(f"🆔 Hypothesis #{hypothesis.id}\n", style="bold cyan")
        content.append("\n")
        ...
        if hypothesis.selected:
            content.append("✓ Currently Selected\n", style="bold green")
        else:
    ...
    def action_edit(self) -> None:
        ...
        if hypothesis_list.highlighted is None:
            self.app.notify("⚠️ No hypothesis selected", severity="warning")
            return
    ...
    def action_save_edit(self) -> None:
        ...
        if not new_text:
            self.app.notify("⚠️ Hypothesis text cannot be empty", severity="warning")
            return
        ...
        self.app.notify("✓ Hypothesis updated", severity="information")
    ...
    def _save_priority(self) -> None:
        ...
        try:
            ...
            if priority < 0 or priority > 10:
                self.app.notify("⚠️ Priority must be between 0 and 10", severity="warning")
                return
        except ValueError:
            self.app.notify("⚠️ Invalid priority value", severity="warning")
            return
        ...
        self.app.notify(f"✓ Priority set to {priority}", severity="information")
    ...
    def _add_custom_hypothesis(self) -> None:
        ...
        self.app.notify("✓ Custom hypothesis added", severity="information")
    ...
    def action_confirm(self) -> None:
        ...
        if not selected:
            self.app.notify(
                "⚠️ No hypotheses selected. Please select at least one hypothesis.",
                severity="warning",
                timeout=3
            )
            return
