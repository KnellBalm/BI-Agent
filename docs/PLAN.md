...
- [ ] `/intent` 입력 시 **LLM 기반 상세 분석 파이프라인(Pipeline)** 제안 로직
- [ ] 업종별/분석 테마별 가설 템플릿 엔진 구축
- [ ] 분석 결과에서 기대되는 비즈니스 가치(ROI) 사전 시뮬레이션
#### **Step 8. 사고 과정 시각화 (Thinking CoT)**
- [ ] 에이전트 간 내부 메시지 버스(`AgentMessageBus`) 실시간 노출 패널
- [ ] LLM의 "생각 중..." 단계를 구체적인 한국어 상태 메시지로 변환
- [ ] 장시간 분석 시 단계별 체크마크 및 진행률 인디케이터
#### **Step 9. 사용자 정렬 (Alignment)**
- [ ] 생성된 가설 중 사용자가 선택/수정할 수 있는 `HypothesisScreen` 구축
- [ ] 분석 제약 조건(기간, 지역, 카테고리) 추가 입력 워크플로우 (`ConstraintScreen`)
- [ ] 에이전트의 제안에 대한 '승인/수정/반려' 단축키 시스템

---

### [Phase 4: 리포트 조립 및 인터랙티브 설계]
#### **Step 10. 최적 쿼리 생성 (Querying)**
- [ ] 가설 검증용 SQL 자동 생성 및 DB 문법 호환성 체크
- [ ] 실행 오류 시 에러 로그 기반 자동 쿼리 수정 루프 (Self-healing)
- [ ] 복잡한 연산 시 Pandas Transformation 코드 자동 생성 및 실행
