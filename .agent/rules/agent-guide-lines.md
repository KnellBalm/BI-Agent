...
- **Korean First Policy**: 사용자와의 모든 상호작용, 코드 주석, **모든 제안(Proposal) 및 계획(Plan)**은 반드시 **한국어**를 사용한다.
...
- **Vibe Coding (Steel Thread)**: 복잡한 아키텍처 구현 시, 전체를 한꺼번에 짜지 않고 **"동작하는 최소한의 수직 단면(Vertical Slice)"**을 먼저 완성하여 빠른 피드백 루프를 확보한다. (가장 얇은 강철 실선을 먼저 긋고 살을 붙인다.)
